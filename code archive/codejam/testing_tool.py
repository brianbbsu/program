...
def Won(vases, case):
  ...
  if len(win_vases_idx) > 1:
    print("failed case {}: vases {} all have {} tokens, so "
          "nobody wins.".format(case, [v + 1 for v in win_vases_idx], min_cnt),
          file=sys.stderr)
    return False
  ...
  for p in win_vase:
    if win_vase.count(p) > 1:
      print("failed case {}: winning vase {} had more than "
            "one token for player {}.".format(case, win_vase_idx + 1, p),
            file=sys.stderr)
      return False

  if npeople not in win_vase:
    print("failed case {}: I don't have a token in the "
          "winning vase {}".format(case, win_vase_idx + 1),
          file=sys.stderr)
    return False
  return True
...
class JudgeSingleCase:
  ...
  def Judge(self):
    ...
    return Won(self.vases, self.case), None
