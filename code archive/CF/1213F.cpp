...
{
    vis[now] = 1;
    for(ll k:v3[now])if(!vis[k])dfs3(k);
    dt.pb(now);
}
...
int main()
{
    ...
    reverse(ALL(dt));
    ll git = 0;
    for(ll t:dt)if(!vis[t])dfs2(t, git++);

    REP1(i, n)for(ll k:v[i])if(g[i] != g[k])v3[g[i]].pb(g[k]);

    dt.clear();
    FILL(vis, 0);
    for(int i = 0;i < git;i ++)if(!vis[i])dfs3(i);
    ...
    cout<<"YES"<<endl;
    REP(i, SZ(dt))ans[dt[i]] = 'a' + min(25LL, i);
    REP1(i, n)cout<<ans[g[i]];
    cout<<endl;
}
